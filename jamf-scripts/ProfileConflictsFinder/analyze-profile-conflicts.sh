...
run_analysis() {
    ...
    # get current state of FAIL_RECIPES_WITHOUT_TRUST_INFO key in autopkg preferences
    local fail_recipes_without_trust_info
    fail_recipes_without_trust_info=$(defaults read com.github.autopkg FAIL_RECIPES_WITHOUT_TRUST_INFO 2>/dev/null)
    # Temporarily set FAIL_RECIPES_WITHOUT_TRUST_INFO to false to allow running untrusted recipe
    ...
    # echo out the command for debugging purposes
    echo "Running AutoPkg command:"
    printf '  %s\n' "${autopkg_cmd[@]}"
    ...
    # Restore original FAIL_RECIPES_WITHOUT_TRUST_INFO setting
    if [[ -n "$fail_recipes_without_trust_info" ]]; then
        defaults write com.github.autopkg FAIL_RECIPES_WITHOUT_TRUST_INFO -bool "$fail_recipes_without_trust_info"
    else
        defaults delete com.github.autopkg FAIL_RECIPES_WITHOUT_TRUST_INFO 2>/dev/null
    fi
